...
| ignoreGroups       | number   | 0       | Indicates the number of matching groups to ignore in the replacement. Can be used e.g. to implement a non-capturing lookbehind. Note that when the value is > 0 (when groups should be ignored), it expects a total amount of groups in the RegExp of `ignoreGroups` + 1                                                                                  |
